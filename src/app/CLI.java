...
public class CLI {
	...
	/**
	 * @brief Affiche les résultats de l’évaluation de la qualité d'une image
	 *        débruitée par rapport à l’image originale.
	 * @author Emma
	...
	/**
	 * @brief Traite les arguments de la ligne de commande pour configurer un
	 *        débruitage.
	 * @author Paul
	...
	private static void traiterArgumentsCommande(String[] args) {
		...
		double sigma = 20.0;
		int taillePatch = 7;
		int tailleFenetre = TAILLE_FENETRE_DEFAUT;
		...
		for (int i = 0; i < args.length; i++) {
			...
			case "--shrink", "-s" -> {
				if (i + 1 < args.length) {
					String type = args[++i].toLowerCase();
					if (type.equals("v"))
						shrinkType = "VisuShrink";
					else if (type.equals("b"))
						shrinkType = "BayesShrink";
					else {
			...
			case "--fenetre-size", "-f" -> {
				if (i + 1 < args.length) {
					try {
						tailleFenetre = Integer.parseInt(args[++i]);
						if (tailleFenetre < TAILLE_FENETRE_MIN || tailleFenetre > TAILLE_FENETRE_MAX) {
							System.err.println("Erreur: Taille de fenêtre doit être comprise entre " 
									+ TAILLE_FENETRE_MIN + " et " + TAILLE_FENETRE_MAX + " pixels");
							afficherAide();
							return;
						}
					} catch (NumberFormatException e) {
						System.err.println("Erreur: Valeur de taille de fenêtre non valide: " + args[i]);
						afficherAide();
						return;
					}
				} else {
					System.err.println("Erreur: Taille de fenêtre manquante pour --fenetre-size");
					afficherAide();
					return;
				}
			}
		...
		try {
			...
			System.out.println(
					"Paramètres: méthode " + methodeTexte + ", seuil " + (thresholdType.equals("Dur") ? "Hard" : "Soft")
			...
			Img xR = debruiteur.imageDen(xB, shrinkType, thresholdType, sigma, taillePatch, tailleFenetre, !isGlobal);
			...
			String outputImagePath = "data/xR/" + baseName + "_" + methodName + "_" + threshold + "_" + shrink
					+ extension;
	...
	private static void afficherAide() {
		...
		System.out.println(
				"  --image, -i <nom>         Nom de l'image (dans data/x0/)                     [Obligatoire]");
		System.out
				.println("  --global, -g              Utilise la méthode de débruitage globale           [Facultatif]");
		System.out.println("  --local, -l               Utilise la méthode de débruitage locale            [Défaut]");
		System.out.println(
				"  --threshold, -t <type>    Type de seuillage: 'hard' ou 'soft'                [Défaut: 'hard']");
		System.out.println(
				"  --shrink, -s <type>       Type de seuillage adaptatif: 'v' ou 'b'            [Défaut: 'v']");
		System.out.println("                            ('v' = VisuShrink, 'b' = BayesShrink)");
		System.out.println(
				"  --sigma, -sig <valeur>    Écart-type du bruit                                [Défaut: 20.0]");
		System.out
				.println("  --patch-size, -p <taille> Taille des patchs (entier impair)                  [Défaut: 7]");
		System.out
		.println("  --fenetre-size, -f <taille> Taille des fenêtres (mode local)                  [Défaut: 250]");
...
//            
//            DebruiteurImage debruiteur = new DebruiteurImage();
//            Img xR = debruiteur.imageDen(xB, typeSeuil, fonctionSeuillage, sigma, taillePatch, TAILLE_FENETRE_DEFAUT, modeLocal);
//            
...
//  
	...
	/**
	 * @brief Demande à l'utilisateur de saisir une taille de patch parmi des
	 *        options valides.
	 * @author Emma
...
//    
	 ...
	 * @author Paul
	 * @param folderPath Le chemin absolu ou relatif vers le dossier contenant les
	 *                   fichiers.
	 * @return Un tableau de chaînes de caractères contenant les noms des fichiers
	 *         image trouvés. Retourne un tableau vide si le dossier est vide,
	 *         invalide, ou ne contient aucune image.
	 */
